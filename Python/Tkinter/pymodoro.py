...
class Pymodoro:
    def __init__(self, master):
        ...
        # watches for changes made to itself and calls self.build timer whenever there is a change  # noqa E501
    ...
    def build_buttons(self):
        ...
        self.start_button = tkinter.Button(
            buttons_frame, text='Start', command=self.start_timer)

        self.stop_button = tkinter.Button(
            buttons_frame, text='Stop', command=self.stop_timer)
    ...
    def build_timer(self, *args):
        timer = tkinter.Label(
            self.mainframe, text=self.timer_text.get(), font=('Helvetica', 36))
    ...
    def alert(self, *args):
        if not self.time_left.get():
            messagebox.showinfo('TIMER DONE', 'Congratulations! You\'ve Completed Your Pymodoro!')  # noqa E501
    ...
    def update(self):
        ...
        if self.running and time_left:
            ...
            # this pads zeros to 2 digits, like so, 0 becomes 00
            ...
            self.timer_text.set('{:0>2}:{:0>2}'.format(minutes, seconds))
            self.stop_timer()
        # after is a function of roor that fires a given function after a certain amount of time  # noqa E501
